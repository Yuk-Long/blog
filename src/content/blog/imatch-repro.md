...
### train_fullres.py（全分辨率版）

```python
"""全分辨率训练——不用 max_pixels 限制，正常看图像"""
...
model.train(); step=0; dev=f"cuda:{GPU}"
...
for ep in range(EPOCHS):
    ...
    for i, s in enumerate(train_raw):
        ...
        if not os.path.exists(ip): ip = os.path.join(IMG_DIR, s["img_path"].lower())
        img = Image.open(ip).convert("RGB")
        score = sum(s["total_score"])/3; lb = letter(score)
        msgs = [
            {"role":"system","content":[{"type":"text","text":SYS}]},
            {"role":"user","content":[{"type":"image","image":img},{"type":"text","text":'Rate: "' + s["prompt"] + '"'}]},
        ]
        ...
        if (i+1)%ACC==0:
            ...
            if step%20==0:
                print(f"  step{step:5d}  loss={running/20:.4f}"); running=0.0
            if step%500==0:
...
用 `max_pixels=256×28²` 限制图像分辨率训练 2 个 epoch 后，验证集评估结果：

| 指标 | 值 | 说明 |
|------|-----|------|
| SRCC | -0.0131 | 接近零，无相关性 |
| PLCC | 0.0007 | 接近零，无相关性 |
...
模型几乎对所有输入都输出相同的字母 J/K，对应分数约 3.3，完全丧失了区分能力。

### 原因

Qwen2.5-VL 的 ViT 视觉编码器将图像切分成固定大小的 patch。`max_pixels=256×28²` 下每张图只有约 **64 个 patches**，图像细节被严重压缩。ViT 实际上"看不清"图中物体是否存在，模型只能学到最安全的策略：永远猜中间区域的字母 J/K，对应分数 ~3.3——恰好是训练集总分的分布中心。

这不是过拟合，而是**信息瓶颈**——视觉通路提供的信号太弱，语言模型被迫忽略图像、只输出统计上最安全的值。

### 解决方案

去掉 `max_pixels` 限制，让 processor 使用默认分辨率（约 `1280×28²` pixels，~1600 patches）。代价是显存占用从 ~25GB 升到 ~35GB，但 5880 Ada 的 48GB 完全够用。全分辨率重训后模型才能正确建立图像与评分之间的关联。

> **教训**：限制分辨率是显存不够时的权宜之计，但在物体级判断任务中，视觉信息不能被过度压缩。能用全分辨率就不要降。
